...
import { IGNORED_NODE, NodeType } from '../node_modules/rrweb-snapshot/es/rrweb-snapshot.js';
...
var mirror2 = {
    ...
    removeNodeFromMap: function (n) {
        var id = n.__rsn && n.__rsn.id;
        delete mirror2.map[id];
        if (n.childNodes) {
            n.childNodes.forEach(function (child) {
                return mirror2.removeNodeFromMap(child);
            });
        }
    },
    has: function (id) {
...
function isIframeINode2(node) {
    if (!node.__rsn) {
        return false;
    }
    return (node.__rsn.type === NodeType.Element && node.__rsn.tagName === 'iframe');
}
function getBaseDimension(node) {
    var _a, _b;
    ...
    var frameBaseDimension = getBaseDimension(frameElement);
    return {
...
export { TreeIndex, getBaseDimension, getWindowHeight, getWindowWidth, hookSetter, isAncestorRemoved, isBlocked, isIframeINode, isIframeINode2, isIgnored, isTouchEvent, iterateResolveTree, mirror, mirror2, needCastInSyncMode, on, patch, polyfill, queueToResolveTrees, throttle };

...
import { rebuild, buildNodeWithSN, NodeType } from '../../node_modules/rrweb-snapshot/es/rrweb-snapshot.js';
import { ReplayerEvents, EventType, IncrementalSource, MediaInteractions, MouseInteractions } from '../types.js';
import { polyfill as polyfill$1, mirror2, isIframeINode2, queueToResolveTrees, iterateResolveTree, getBaseDimension, TreeIndex } from '../utils.js';
...
var Replayer = (function () {
        ...
        this.emitter.on(ReplayerEvents.Flush, function () {
            ...
            try {
                for (var _e = __values(_this.fragmentParentMap.entries()), _f = _e.next(); !_f.done; _f = _e.next()) {
                    ...
                    if (parent.__rsn.type === NodeType.Element &&
    ...
    Replayer.prototype.collectIframeAndAttachDocument = function (collected, builtNode) {
        if (isIframeINode2(builtNode)) {
            var mutationInQueue = this.newDocumentQueue.find(function (m) { return m.parentId === builtNode.__rsn.id; });
            if (mutationInQueue) {
    ...
    Replayer.prototype.applyIncremental = function (e, isSync) {
        ...
        switch (d.source) {
            ...
            case IncrementalSource.MouseInteraction: {
                ...
                var target = mirror2.getNode(d.id);
                if (!target) {
    ...
    Replayer.prototype.applyMutation = function (d, useVirtualParent) {
        ...
        var appendNode = function (mutation) {
            ...
            if (isIframeINode2(target)) {
                var mutationInQueue_1 = _this.newDocumentQueue.find(function (m) { return m.parentId === target.__rsn.id; });
                ...
                if (target.contentDocument) {
                    var _a = target.contentDocument, documentElement = _a.documentElement, head = _a.head;
    ...
    Replayer.prototype.moveAndHover = function (d, x, y, id) {
        var target = mirror2.getNode(id);
        if (!target) {
            return this.debugNodeNotFound(d, id);
        }
        var base = getBaseDimension(target);
        var _x = x + base.x;
        var _y = y + base.y;
